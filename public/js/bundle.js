...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1Ijoic2VtZW5vdjE5ODkiLCJhIjoiY2tiZ2QyMmI1MDZ1MjJ5cDNvcTJmMjN6cCJ9.HxC8s0vYpTzAox4WRl01NQ';
  var map = new mapboxgl.Map({
    container: 'map',
    //отобразит карту в элементе с id map
    style: 'mapbox://styles/semenov1989/ckbgdoigr1s651isakdkbwbam',
    scrollZoom: false //center: [-118.113491, 34.111745], //где карта будет отцентрована по-умолчанию
    //zoom: 4,
    //interactive: false,
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // Добавляем маркер

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom' //низ маркера будет указывать на нужную точку

    }).setLngLat(loc.coordinates).addTo(map); //присваиваем координаты локации маркеру и добавляем на карту
    // Add popup
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 7:
            ...
            (0, _alert.showAlert)('error', _context.t0.response.data.message); //сообщение из нашего json ответа
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            //тернерный оператор в url: при типе пароль - для изменения пароля, иначе - обновление данных
...
if (mapBox) {
  ...
  (0, _mapbox.displayMap)(locations); //вызываем функцию для отображения карты и передаем в нее локации
}
...
if (userDataForm) userDataForm.addEventListener('submit', function (e) {
  ...
  (0, _updateSettings.updateSettings)(form, 'data'); //1 - аргумент объект с данными, 2 - тип (для функции - пароль или данные)
});
